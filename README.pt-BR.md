...
*   Obter o valor do **_Tenacy Namespace_** necessário para criação da _imagem Docker_. Para maiores informações, consulte a documentação oficial: _[Noções Básicas sobre Namespaces do Serviço Object Storage](https://docs.oracle.com/pt-br/iaas/Content/Object/Tasks/understandingnamespaces.htm)_
...
*   Criar a **_TAG _**utilizada para construção da _imagem _que deve obedecer ao padrão:
...
*   Para maiores informações de como gerar o **_Token de Autenticação, _**consulte a documentação oficial: _[Obtendo um Token de Autenticação](https://docs.oracle.com/pt-br/iaas/Content/Registry/Tasks/registrygettingauthtoken.htm)_
...
*   Realizar o _login _no serviço _OCIR_, informando seu _nome de usuário_ e o **_Token de Autenticação_** que foi criado. Lembrando que neste exemplo, estamos utilizando os serviços do _OCI _presentes na região _São Paulo, Brazil (GRU)_.
...
*   Enviar a_ imagem Docker_ ao serviço _OCIR_:
...
10. Criar a infraestrutura no _OCI _utilizando os scripts _Terraform_.
...
*   Os recursos que serão criados são:
    *   [Serviço de Log](https://docs.oracle.com/pt-br/iaas/Content/Logging/Concepts/loggingoverview.htm):
        *   Log Group: fotogal_loggroup
        *   Log: fotogal_customlog_app
    *   [VCN](https://docs.oracle.com/pt-br/iaas/Content/Network/Tasks/managingVCNs.htm):
        *   Nome: vcn-oke
        *   CIDR Block: 10.39.0.0/16
    *   [Subrede Pública](https://docs.oracle.com/pt-br/iaas/Content/Network/Tasks/managingVCNs.htm):
        *   Nome: subnpub-1_vcn-oke
        *   CIDR Block: 10.39.1.0/24
    *   [Subrede Privada](https://docs.oracle.com/pt-br/iaas/Content/Network/Tasks/managingVCNs.htm):
        *   Nome: subnprv-1_vcn-oke
        *   CIDR Block: 10.39.2.0/24
    *   [Internet Gateway](https://docs.oracle.com/pt-br/iaas/Content/Network/Tasks/managingIGs.htm):
        *   Nome: igw_vcn-oke
    *   [NAT Gateway](https://docs.oracle.com/pt-br/iaas/Content/Network/Tasks/NATgateway.htm):
        *   Nome: natgw_vcn-oke
    *   [Service Gateway](https://docs.oracle.com/pt-br/iaas/Content/Network/Tasks/servicegateway.htm):
        *   Nome: srvgw_vcn-oke
    *   [Cluster Kubernetes](https://docs.oracle.com/pt-br/iaas/Content/ContEng/Concepts/contengoverview.htm):
        *   Nome: oke-fotogal
        ...
        *   2x Worker Nodes
            *   OCPU: 2
            *   Mem: 4 GB
    *   [Object Storage](https://docs.oracle.com/pt-br/iaas/Content/Object/Concepts/objectstorageoverview.htm) (private bucket):
        *   Nome: fotogal_bucket_images
    *   [Tabelas NoSQL](https://docs.oracle.com/pt-br/iaas/nosql-database/index.html):
        *   fotogal_ntable_users
        *   fotogal_ntable_authsession
        *   Fotogal_ntable_images
11. Criar as configurações necessárias para conexão ao cluster _Kubernetes _a partir do **_kubectl_**:
...
*   Após o cluster _Kubernetes _ser criado, será necessário obter o _OCID _do mesmo:

        ```
[darmbrust@oci-dev fotogal]$ oci ce cluster list 
...
*   Preencha o comando abaixo com o respectivo valor _OCID _do _cluster Kubernetes_ que foi criado:

        ```
[darmbrust@oci-dev fotogal]$ oci ce cluster create-kubeconfig 
   --cluster-id <CLUSTER OCID> --file $HOME/.kube/config --region sa-saopaulo-1 --token-version 2.0.0
...
13. Criar um _“segredo”_ para permitir que o_ cluster Kubernetes_ acesse o repositório de _imagens Docker _no _OCI_:

    ```
[darmbrust@oci-dev fotogal]$ kubectl create secret docker-registry fotogal-ocir-secret
   --docker-server=gru.ocir.io 
   --docker-username='idreywyoj0pu/oracleidentitycloudservice/daniel.armbrust@oracle.com'   
   --docker-password='<TOKEN DE AUTENTICAÇÃO>'
```
...
14. Acertar o arquivo **_iac/yaml/fotogal-deploy.yaml_** com o caminho da _imagem Docker_ de acordo com as informações do _OCIR _criadas no seu _Tenancy_:
